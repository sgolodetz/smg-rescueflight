...
if [ $# -lt 4 ]
then
  echo "Usage: detect_gta_im_skeletons.sh <sequence name> <method tag> {gt|lcrnet|maskrcnn} {gt|lcrnet|xnect} [args]"
  exit 1
fi
...
# Start the skeleton detection process.
echo "Detecting skeletons for $1 ($2)"

# If the output directory already exists, early out.
if [ -e "$sequence_dir/skeletons/$2" ]
then
  echo "- Found skeletons/$2 directory: skipping"
  exit 0
fi
...
conda activate smglib
python ../mapping/scripts/run_octomap_mapping_server.py --detect_skeletons -p wait --use_received_depth --octree_voxel_size=0.1 "${@:5}" > /dev/null 2>& 1 &
conda deactivate
