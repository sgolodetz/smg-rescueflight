...
if [ $# -lt 4 ]
then
  echo "Usage: reconstruct_gta_im_scene_offline.sh <sequence name> <method tag> {gt|dvmvs|mvdepth} {gt|lcrnet|lcrnet-smpl|maskrcnn|nomask|xnect|xnect-smpl} [args]"
  exit 1
fi
...
if [ "$3" == "gt" ]
then
  echo "$detect_skeletons_flag" | xargs python ../mapping/scripts/run_open3d_mapping_server.py --batch --debug --output_dir="$sequence_dir/recon" -p wait --reconstruction_filename="$2.ply" --save_reconstruction --use_received_depth "${@:5}" > /dev/null 2>&1 &
else
  echo "$detect_skeletons_flag" | xargs python ../mapping/scripts/run_open3d_mapping_server.py --batch --debug --output_dir="$sequence_dir/recon" -p wait --reconstruction_filename="$2.ply" --save_reconstruction --depth_estimator_type="$3" "${@:5}" > /dev/null 2>&1 &
fi
server_pid="$!"
conda deactivate
