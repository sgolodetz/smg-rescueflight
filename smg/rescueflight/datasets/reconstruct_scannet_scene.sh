...
then
  ...
  python ../mapping/scripts/run_open3d_mapping_server.py --batch --debug -p wait --output_dir="$sequence_dir/recon" --save_reconstruction --use_received_depth "${@:3}" > /dev/null 2>&1 &
  sleep 5
...
then
  echo "Initialising mapping server..."
  python ../mapping/scripts/run_open3d_mapping_server.py --batch --debug -p wait --output_dir="$sequence_dir/recon" --save_reconstruction "${@:3}" > /dev/null 2>&1 &
  sleep 5
  echo "Reconstructing $1..."
