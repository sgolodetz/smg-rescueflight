...
if [ $# -ne 1 ]
then
  echo "Usage: evaluate_ohm_sequences.sh <sequence list file>"
  exit 1
fi
...
do
  ./evaluate_ohm_sequence.sh "$sequence_name"
done
...
root_dir=`./determine_dataset_root_dir.sh ohm`
conda activate smglib
python make_ohm_tables.py -r "$root_dir" -s "$1"
