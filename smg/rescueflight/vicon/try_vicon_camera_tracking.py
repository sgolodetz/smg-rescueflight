...
def estimate_source_pose(*, image: np.ndarray, intrinsics: Tuple[float, float, float, float],
    ...
    if subject_from_source is not None:
        ...
        # TODO
        world_from_subject: Optional[np.ndarray] = None

        # TODO
        if world_from_subject is not None:
            # TODO
            return world_from_subject @ subject_from_source
    ...
    return relocaliser.estimate_pose(image, intrinsics, draw_detections=True)


def render_window(*, image: np.ndarray, image_renderer: OpenGLImageRenderer, viewing_pose: np.ndarray,
                  window_size: Tuple[int, int], world_from_source: Optional[np.ndarray]) -> None:
    # Clear the window.
